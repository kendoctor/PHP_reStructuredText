...
なんとなくのサンプルとしてrst2html.phpがあるのでとりあえずHTMLへの書き出しは出来ますが
イベント名の変更等があるのでこれを元にするべきではありません。（parserからdocument treeを作ってtreeに対して処理を行うほうが
汎用性が高くなるので）


現状のPHPで一番良いreStructuredTextを扱う手段はpythonのrst2xmlで変換した
XMLに対してPHP側で処理を行う事です。
